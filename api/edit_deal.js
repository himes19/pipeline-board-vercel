  ...
  try {
    ...
    if (field === 'stage') {
      ...
      if (existingTs && existingTs.length > 0) {
        await executeDB(`UPDATE entry_fields SET value = '${now}' WHERE entry_id = '${entryId}' AND field_id = '${STAGE_CHANGED_AT_FIELD}'`);
      } else {
    ...
    if (!fieldId) {
      res.status(400).json({ error: 'Unknown field: ' + field }); return;
    }
    const existing = await queryDB(
      `SELECT id FROM entry_fields WHERE entry_id = '${entryId}' AND field_id = '${fieldId}' LIMIT 1`
    );
    if (existing && existing.length > 0) {
